...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            if i == 0:
                agent.color = np.array([1, 0.5, 0.5])  # 红色
            elif i == 1:
                agent.color = np.array([0.99, 0.38, 0.28])  # 粉色
            elif i == 2:
                agent.color = np.array([0.59, 0.98, 0.59])  # 绿色
            elif i == 3:
                agent.color = np.array([0, 0.65, 0.99])  # 蓝色
    ...
    def reward(self, agent, world):
        ...
        if agent.name == "agent_0":  # 如果是虚拟领航者
            ...
            rew -= distance * 1.2
        # formation
        if abs(a1[0] - a0[0]) < 0.2 and a1[1] < a0[1]:
            rew += 0.3
        else:
            rew -= 0.4
        if abs(a2[0] - a1[0]) < 0.2 and a2[1] < a1[1]:
            rew += 0.3
        else:
            rew -= 0.4
        if abs(a3[0] - a2[0]) < 0.2 and a3[1] < a2[1]:
            rew += 0.3
        else:
            rew -= 0.4
        # 障碍规避部分
